...
export const verify = () => async function verify (req, res) {
 ...
 *
 * When 2FA isnt setup, the result will include data required to start the setup.
 */
export const status = () => async function status (req, res) {
  try {
 ...
 */
export const setup = () => async function setup (req, res) {
  try {
    const data = insecurity.authenticatedUsers.from(req)
    if (!data) {
 ...
 */
export const disable = () => async function disable (req, res) {
  try {
    const data = insecurity.authenticatedUsers.from(req)
    if (!data) {
      throw new Error('Need to login before setting up 2FA')
