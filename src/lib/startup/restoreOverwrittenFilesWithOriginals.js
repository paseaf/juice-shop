...
const restoreOverwrittenFilesWithOriginals = async () => {
  await copyFile(path.resolve('src/data/static/legal.md'), path.resolve('ftp/legal.md'))

  if (await exists(path.resolve('frontend/dist'))) {
    await copyFile(
      path.resolve('src/data/static/owasp_promo.vtt'),
      path.resolve('frontend/dist/frontend/assets/public/videos/owasp_promo.vtt')
    )
  }

  try {
    const files = await glob(path.resolve('src/data/static/i18n/*.json'))
    await Promise.all(
      files.map((filename) => copyFile(filename, path.resolve('i18n/', filename.substring(filename.lastIndexOf('/') + 1))))
    )
  } catch (err) {
    logger.warn('Error listing JSON files in src/data/static/i18n folder: ' + err.message)
  }
}
