...
const restoreOverwrittenFilesWithOriginals = async () => {
  ...
  if (await exists(path.resolve(__dirname, '../../frontend/dist'))) {
    await copyFile(path.resolve(__dirname, '../../data/static/owasp_promo.vtt'), path.resolve(__dirname, '../../frontend/dist/frontend/assets/public/videos/owasp_promo.vtt'))
  }
